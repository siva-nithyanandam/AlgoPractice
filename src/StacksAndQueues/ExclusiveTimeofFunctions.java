...
/**
 * Author - Sivaprakash Nithyanandam Timestamp - 6/20/2021  8:46 PM
 *
...
public class ExclusiveTimeofFunctions {
  ...
  public static void main(String[] args) {
    ...
    System.out.println(Arrays.toString(o.exclusiveTime(2, Arrays.asList("0:start:0","0:start:2","0:end:5","1:start:6","1:end:6","0:end:7"))));
    System.out.println(Arrays.toString(o.exclusiveTime(1, Arrays.asList("0:start:0","0:start:2","0:end:5","0:start:6","0:end:6","0:end:7"))));
  }

  public int[] exclusiveTime_faster(int n, List<String> logs) {
    int[] times = new int[n];
    int[] stack = new int[logs.size() / 2];
    int si = -1;
    int i;
    int id, time, prevTime = 0;
    for (String log : logs) {
      i = 0;
      id = 0;
      while (log.charAt(i) != ':') {
      ...
      if (log.charAt(i + 1) == 's') {
        time = getIntAfter(log, i + 7);
        if (si != -1)
          times[stack[si]] += time - prevTime;
        stack[++si] = id;
      } else {
        time = getIntAfter(log, i + 5) + 1;
        times[stack[si--]] += time - prevTime;
      }
      prevTime = time;
    }
  ...
  private int getIntAfter(String s, int i) {
    int num = 0;
    while (i != s.length()) {
  ...
  public int[] exclusiveTime(int n, List<String> logs) {
    ...
    for (String str : logs) {
      ...
      if (arr[1].equals("start")) {
        ss.push(new int[]{dur, 0});
      } else {
        int[] start = ss.pop();
        int time = dur - start[0] + 1;
        res[id] += time - start[1];
        if (!ss.isEmpty()) {
          ss.peek()[1] += time;
        }
